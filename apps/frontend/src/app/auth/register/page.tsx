      ...
      <motion.div className="glass-card p-xl rounded-xl ai-glow" variants={fadeUp}>
        ...
        <form className="space-y-md" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="space-y-xs group">
            ...
            <div className="relative flex items-center border border-outline-variant bg-black rounded p-sm transition-all focus-within:border-primary focus-within:shadow-[0_0_15px_rgba(128,131,255,0.1)]">
            ...
            <div className="grid grid-cols-2 gap-sm">
              {roles.map((role) => (
                <label key={role.value} className="cursor-pointer group">
                  <input type="radio" name="role" value={role.value} defaultChecked={role.value === 'Student'} className="hidden peer" />
                  <div className="flex items-center justify-center gap-sm p-sm border border-outline-variant rounded bg-black/50 peer-checked:border-primary peer-checked:bg-primary-container/10 transition-all hover:bg-surface-container-high">
                    <Icon className="text-outline group-hover:text-on-surface peer-checked:text-primary" size="18px">{role.icon}</Icon>
                    <span className="font-label-md text-on-surface-variant peer-checked:text-on-surface">{role.label}</span>
                  </div>
                </label>
              ))}
            </div>
          ...
          <button
            type="submit"
            className="w-full bg-primary hover:bg-primary-fixed-dim active:scale-[0.98] text-on-primary font-headline-md py-sm rounded-lg transition-all ai-glow mt-lg"
          >
            Create Account
          </button>
        </form>
